            ...
            <nav id="secondary_nav">
                <div class="container">
                    <ul class="clearfix">
                        <li>
                            <a class="active">
                                {{ ucwords($user->name) }}  {{ getLanguageValue('Profile') }}
                            </a>
                        </li>
                        @switch($user->level) @case('doctor')
                        <li>
                            <a href="#consultant">
                        ...
                        @break @case('pharmacy') @break @default @endswitch
                    </ul>
                </div>
            </nav>
            <div id="section_1">
    ...
    @include('includes.errors')
                    ...
                    <div class="profile">
                        <div class="row">
                            <div class="col-lg-5 col-md-4 text-center">
                                <figure>
                                    <a href="/images/users/{{( $user->profile_image) ? $user->profile_image : 'undraw_profile_pic_ic5t.png'    }}" target="_blank">
                                        <img src="/images/users/{{ ($user->profile_image) ? $user->profile_image : 'undraw_profile_pic_ic5t.png'    }}" class="img-fluid" style="max-height: 300px;">
                                    </a>
                                </figure>
                                <h6 id="infoSpan">
                                    {{ ($user->about) ? $user->about : 'No Info Yet' }}
                                </h6>
                            </div>
                            <div class="col-lg-7 col-md-12">
                                <small>{{ ucwords($user->level) }}</small>
                                <h1>{{ ucwords($user->name) }}</h1>
                                <span class="rating">
                                    @foreach (range(1, intval($user->userAverageRating(10))) as $star)
                                        <i class="icon_star
                                        {{ ($loop->last) ? '' :'voted'}}
                                        "></i>
                                    @endforeach
                                    <small>({{ $user->userSumRating }})</small>


                                </span>
                                <ul class="statistic">
                                    <li>{{ $user->viewed+1 }} {{ getLanguageValue('Views') }}</li>
                                    <li>124 Patients</li>
                                </ul>
                                <ul class="contacts">

                                    <li>
                                        <h6>{{ getLanguageValue('Address') }}
                                        </h6>
                                        {{ $user->address }}
                                    </li>
                                    <li>
                                        <h6>{{ getLanguageValue('Phone') }}
                                        </h6>
                                        {{ $user->phone }}
                                    </li>
                                    <li>
                                        <h6>
                                            {{ getLanguageValue('Email') }}
                                        </h6>
                    ...
                    <div class="wrapper_indent">
                        <p>Sed pretium, ligula sollicitudin laoreet viverra, tortor libero sodales leo, eget blandit nunc tortor
                            eu nibh. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Phasellus hendrerit. Pellentesque
                            aliquet nibh nec urna. In nisi neque, aliquet vel, dapibus id, mattis vel, nisi. Nullam mollis.
                            Phasellus hendrerit. Pellentesque aliquet nibh nec urna. In nisi neque, aliquet vel, dapi.</p>
        ...
        <aside class="col-xl-4 col-lg-4" id="sidebar">
            <div class="box_general_3 booking">
                <form>
                    <div class="title">
                        <h3>{{ getLanguageValue('Consultant') }}</h3>
